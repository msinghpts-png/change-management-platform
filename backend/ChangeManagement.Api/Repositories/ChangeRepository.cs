...
public class ChangeRepository : IChangeRepository
{
    ...
    // Intentionally lightweight list query for performance; full graph loading remains in GetByIdAsync/BaseQuery.
    ...
    {
        var duplicateApprovers = incoming.ChangeApprovers
            .GroupBy(a => a.ApproverUserId)
            .Where(g => g.Count() > 1)
            .Select(g => g.Key)
            .ToList();
        if (duplicateApprovers.Count > 0)
        {
            throw new InvalidOperationException($"Duplicate approver ids in update payload: {string.Join(",", duplicateApprovers)}");
        }

        var incomingByUser = incoming.ChangeApprovers
            .ToDictionary(a => a.ApproverUserId, a => a);
    ...
    {
        var duplicateApprovers = incoming.ChangeApprovals
            .GroupBy(a => a.ApproverUserId)
            .Where(g => g.Count() > 1)
            .Select(g => g.Key)
            .ToList();
        if (duplicateApprovers.Count > 0)
        {
            throw new InvalidOperationException($"Duplicate approval approver ids in update payload: {string.Join(",", duplicateApprovers)}");
        }

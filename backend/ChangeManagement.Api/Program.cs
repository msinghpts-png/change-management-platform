...
app.UseExceptionHandler(exceptionApp =>
{
    exceptionApp.Run(async context =>
    {
        ...
        var statusCode = feature?.Error is UnauthorizedAccessException ? StatusCodes.Status403Forbidden : StatusCodes.Status500InternalServerError;
...
");
    ...
    var cabPassword = app.Configuration["SeedCab:Password"] ?? adminPassword;
    ...
    if (cabUser is null)
    {
        dbContext.Users.Add(new User
        {
            ...
            Role = "CAB",
            IsActive = true,
            PasswordHash = PasswordHasher.Hash(cabPassword)
        });
    }
    else
    {
        ...
        if (!PasswordHasher.Verify(cabPassword, cabUser.PasswordHash))
        {
            cabUser.PasswordHash = PasswordHasher.Hash(cabPassword);
        }
    }

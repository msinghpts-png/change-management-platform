...
public class WorkflowEngineTests : IClassFixture<CustomWebApplicationFactory>
{
    ...
    [Fact]
    public async Task Cab_CanApprove()
    {
        await AuthTestHelper.AuthenticateAsAdminAsync(_client);
        var createResponse = await _client.PostAsJsonAsync("/api/changes", new
        {
            Title = "approval-role-success",
            Description = "desc",
            BackoutPlan = "rollback",
            ChangeTypeId = 1,
            PriorityId = 2,
            RiskLevelId = 2,
            PlannedStart = DateTime.UtcNow.AddDays(1),
            approverUserIds = new[] { "22222222-2222-2222-2222-222222222222" },
        ...
        var submit = await _client.PostAsJsonAsync($"/api/changes/{createdId}/submit", new { });
        submit.EnsureSuccessStatusCode();

        await AuthTestHelper.AuthenticateAsCabAsync(_client);
        var approve = await _client.PostAsJsonAsync($"/api/changes/{createdId}/approve", new { comments = "approved" });
